...
### 1. Slack — an agent that joins the thread (Roam — Group Planning Assistant)

**OpenAI + CopilotKit Channels + Exa**

**Roam** is an autonomous group planning and arbitration assistant living directly inside Slack channels and threads. It listens to group discussions, reconciles member preferences, crafts multi-day itineraries, and splits expenses using multimodal vision.

#### ✈️ Multi-Day Itinerary & Group Travel Planning
Mentioning `@roam` in a channel plans multi-day trips with traveler rosters, budget estimates, and native interactive itinerary cards with direct Google Calendar event scheduling:

![Roam 3-Day Osaka Trip Itinerary Card in Slack](assets/screenshots/roam-travel-itinerary.png)

#### 💸 Multimodal Ticket & Receipt Bill Splitting
Uploading an e-ticket, invoice, or receipt image in Slack triggers vision inspection to extract line items and compute fair splits between members directly in the thread:
